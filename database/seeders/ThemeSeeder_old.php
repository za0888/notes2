...
//        1
        ...
        function catsForTheme(User $user, string $themeName, array $catNames) :\Illuminate\Database\Eloquent\Collection
        {
        ...
        catsForTheme($user,"Soft",['LARAVEL','JS','PHP']);
...
//3
        ...
        \Auth::login($user);
        $theme = Theme::create([['name' => 'Dachia', 'created_by_user' => $user->id]]);
        $categories = $theme->categories()
            ->createMany(
                [
                    ['name' => 'PLANTS', 'created_by_user' => $user->id],
                    ['name' => 'HOUSE', 'created_by_user' => $user->id],
                ]);
